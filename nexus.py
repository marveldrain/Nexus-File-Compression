...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print('Usage: python nexus.py <file_to_compress>')
        print('Or create a test file first.')
        # Create a small test file if none provided
        test_data = b'This is a test file for Nexus compression. ' * 1000
        with open('test.txt', 'wb') as f:
            f.write(test_data)
        print('Created test.txt for demo')
        data = test_data
    else:
    ...
    with open('test.nexus', 'wb') as f:
        f.write(packed)
    print('Compressed to test.nexus')